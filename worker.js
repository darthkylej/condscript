...
async function createMeeting(env, wardId, userId, meta = {}, wardName = '') {
  ...
  const r = await query(env,
    `INSERT INTO meetings (ward_id, meeting_date, meeting_time, location, ward_name, is_broadcast, testimony_end_time, created_by)
     VALUES ($1, $2, NULLIF($3, '')::time, NULLIF($4, ''), NULLIF($5, ''), $6, NULLIF($7, '')::time, $8)
     RETURNING id`,
    [wardId, meta.meeting_date, meta.meeting_time || '', meta.location || '', meetingWardName, meta.is_broadcast !== false, meta.testimony_end_time || '', userId]);
  return r.rows[0].id;
}
...
async function getMeetingPayloadById(env, wardId, meetingId) {
  const mr = await query(env,
    `SELECT id, meeting_date, meeting_time, location,
            ...
            invocation, benediction, is_fast_sunday, is_broadcast, testimony_end_time,
...
async function handleUpsertMetadataById(request, env, meetingId) {
  ...
  const allowed = ['ward_name','presiding_name','conducting_name','chorister','organist',
    'invocation','benediction','is_fast_sunday','is_broadcast','testimony_end_time','meeting_date','meeting_time','location',
    'opening_hymn_number','opening_hymn_title',
    'sacrament_hymn_number','sacrament_hymn_title',
    'closing_hymn_number','closing_hymn_title'];
  ...
  allowed.forEach(f => {
    if (meta[f] !== undefined) {
      const valueExpr = ['meeting_time','testimony_end_time'].includes(f) ? `NULLIF($${i}, '')::time` : `$${i}`;
...
async function handleExportCsv(request, env) {
  ...
  const r = await query(env,
    `SELECT m.meeting_date, m.meeting_time, m.location,
            ...
            m.chorister, m.organist, m.invocation, m.benediction, m.is_fast_sunday, m.is_broadcast, m.testimony_end_time,
